class MusicLibraryController
  ...
  def initialize(path="./db/mp3s")
     mp3_to_import= MusicImporter.new(path)
    mp3_to_import.import
  end 
  ...
  def list_songs
   ...
   list.each.with_index{|song,i|  puts "#{i+1}. #{song.artist.name} - #{song.name} - #{song .genre.name}"}
   end
   ...
   def list_songs_by_artist
        ...
        if artist = Artist.find_by_name(input)
         artist.songs.sort{|a,b|  a.name<=>b.name}.each.with_index{|song,i|  puts "#{i+1}. #{song.name} - #{song .genre.name}"}
      end
    end 
